...
CREATE OR REPLACE FUNCTION elemento_a_distancia_X(jugador_consulta REAL[], distancia REAL) RETURNS INTEGER[] AS $BODY$
DECLARE
	jugadores INTEGER[];
...
BEGIN
	...
	jugadores := (SELECT array_agg(candidatos.jugador)
					FROM (SELECT firmas.jugador, distancia_jugadores_fqa_full(jugador_consulta,(SELECT arrays_jugadores(ARRAY[firmas.jugador]))) AS dist
							...
							AND firmas.pivote18 BETWEEN firma_inicial[18] AND firma_final[18]
