...
DROP FUNCTION elemento_a_distancia_x_generados(real[],real);
CREATE OR REPLACE FUNCTION elemento_a_distancia_X_generados(jugador_consulta REAL[], distancia REAL) 
...
BEGIN
	...
	FOR par IN (SELECT candidatos.jugador, candidatos.dist
			FROM (SELECT firmas.jugador, distancia_jugadores_fqa_full(jugador_consulta,(SELECT arrays_jugadores(ARRAY[firmas.jugador]))) AS dist
				...
				AND firmas.firma[55] BETWEEN firma_inicial[55] AND firma_final[55]) candidatos
